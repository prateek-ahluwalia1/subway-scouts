...
@Component({
  selector: 'app-add-award-rate',
  templateUrl: './add-award-rate.component.html',
  styleUrls: ['./add-award-rate.component.scss']
})

export class AddAwardRateComponent implements OnInit {
  ...
  private initializeChargeLists(rates: any): void {
    ...
    // this.chargeListMetroRegional = [
    //   { metro: rates.pf_day, date: 'Day', regional: rates.casual_day, metroName: 'pf_day', regionalName: 'casual_day' },
    //   { metro: rates.pf_night, date: 'Night', regional: rates.casual_night, metroName: 'pf_night', regionalName: 'casual_night' },
    //   { metro: rates.pf_sat, date: 'Saturday', regional: rates.casual_sat, metroName: 'pf_sat', regionalName: 'casual_sat' },
    //   { metro: rates.pf_sun, date: 'Sunday ', regional: rates.casual_sun, metroName: 'pf_sun', regionalName: 'casual_sun' },
    //   { metro: rates.pf_ph, date: 'Public Holiday ', regional: rates.casual_ph, metroName: 'pf_ph', regionalName: 'casual_ph' },
    // ];
  ...
  submitPayRateForm(value, type, el?: HTMLElement): void {
    ...
    if (!this.title || !this.effective_date || !this.level || !this.hours) {
      ...
      if (type == 'submit') {
        ...
        // console.log("Submit payrate", payload)
        this.chargeRateService.sendawardPayRates(payload).subscribe(res => {
          if (res.success) {
        ...
        console.log("Update payrate", payload)
        this.chargeRateService.updateAwardPayRates(payload).subscribe(res => {
          if (res.success) {
            this.toastService.toastNotification(res.message, status);
          }
          else {
