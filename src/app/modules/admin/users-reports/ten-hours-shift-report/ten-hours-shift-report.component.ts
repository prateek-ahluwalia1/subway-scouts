...
@Component({
  selector: 'app-ten-hours-shift-report',
  templateUrl: './ten-hours-shift-report.component.html',
  styleUrls: ['../users-reports.component.scss'],
})
export class TenHoursShiftReportComponent implements OnInit {
  ...
  getStartEnd(event: any) {
    ...
    getReport(){
      ...
      this.reportService.downloadtenHoursShift(data).subscribe(({ success, path }) => {
        if (success) {
          this._commomService.downloadExcelFile(path, `ten_hours-shift-report.xlxs`);
          this.trackAdmin.storeActivity('10 Hours Shift Report', `Download a ten hours shift file`, localStorage.getItem('routerId')).subscribe();
        }
      });
    }
