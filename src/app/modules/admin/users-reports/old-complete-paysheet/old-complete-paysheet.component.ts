...
@Component({
  selector: 'app-old-complete-paysheet',
  templateUrl: './old-complete-paysheet.component.html',
  styleUrls: ['../users-reports.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class OldCompletePaysheetComponent implements OnInit {
  ...
  constructor(public globals: GlobalVariable, private cus: CustomerService,
    ...
    receiveDataFromChild(data: string) {
      ...
      getReports(type) {
        ...
        this.resportService.getoldReports(data).subscribe(({ success, path, data }) => {
    
          if (success && type == 'excel') {
            this._commonService.downloadExcelFile(path, `OldCompletePaysheet.xlxs`)
            this.trackAdmin.storeActivity('Old Complete Paysheet Report', `Download a xlxs file`, localStorage.getItem('routerId')).subscribe(() => {
            })
          }
