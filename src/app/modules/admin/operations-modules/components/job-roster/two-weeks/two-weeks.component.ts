...
@Component({
  ...
  styleUrls: ['./two-weeks.component.scss'],
  providers: [MatMenuTrigger],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TwoWeeksComponent implements OnInit, OnDestroy {
  job_id_match;
  site_id_match;
  ...
  sites = [];
  buit_in_template = []
  //guard single selection
  searchSites

  ///add guard to form when view by guard
  guard_id
  ...
  constructor(
    ...
    public jobroster: JobRosterComponent,
  ...
  ngOnInit(): void {
    ...
    this.sites = []
    this.rosterService.sites$
      .pipe(filter(res => res && res.data),
        takeUntil(this._unsubscribeAll))
      .subscribe((sites: any) => {
        if (this._data.length > 0) {
          this.sites = sites?.data.filter(item => this._data.includes(item.id));
        } else {
              ...
              mainArray.jobRoster.forEach(object => {
                this.sites?.forEach(oldArray => {
                  if (oldArray.id == object.site_id) {
                    const index = oldArray.jobRoster.findIndex(item => item.roster_id == object.roster_id);
                    if (index != -1) {
  ...
  customTemplateModal(day, p: NgbPopover) {
    p.close();
    this.close()
    // const modalRef = this.modalService.open(AddCustomTemplateComponent, { windowClass: 'custom-class', animation: true });
    // this.popover = true
  ...
  editShiftAction(type, job, site?, day?) {
    ...
    if (type == 'edit') {
      ...
      modalRef.componentInstance.site = { site_name: site.site_name, id: site.id, customer_id: site?.customer_id };
  ...
  dragDropApi(data) {
    this.spinner.show()
    this.rosterService.dragDrop(data).subscribe(res => {
      let status = 'Job Roster Operation!'
      if (res.success) {
        this.toast.toastNotification(res.message, status)
        this.jobroster.sendFilterData()
  ...
  shiftCopy(job) {
    ...
    this.shift_paste = !this.shift_paste
    this.copyID = job.roster_id
    this.copyJob = job
  }

  paste(day, id) {
    let data = this.commonService.paste(day, id, this.copyJob.roster_id, this.calenderType, this.copyJob)
    this.savePaste(data);
  }
  ...
  savePaste(data) {
    this.spinner.show()
    this.rosterService.copyShift(data).subscribe(res => {
      let status = 'Job Roster Operation!'
      if (res.success) {
        this.toast.toastNotification(res.message, status)
        this.jobroster.sendFilterData()
  ...
  siteAction(type, id) {
    if (type == 'edit') {
      if (this.calenderType != 'staff') {
        const modalRef = this.modalService.open(CreateSiteComponent, { size: 'xl' });
  ...
  deletedata(id) {
    ...
    this.siteService.delSite(data).subscribe(({ message, success }) => {
      ...
      (error) => {
        this.toast.toastNotification1('Something went wrong. Please contact with support team.', 'Site Operation!');
      }
    );
    this.modalRef.close();
  }
  ...
  trackBySiteId(index: number, item: { id: number, site_name: string }): number {
    return item.id;
  }
