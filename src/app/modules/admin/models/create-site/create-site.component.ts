import { Component, OnInit, ViewChild, ElementRef, Renderer2, Input, ChangeDetectionStrategy,
  HostListener, ChangeDetectorRef } from "@angular/core";
import { FormBuilder, FormControl, FormGroup, Validators, AbstractControl, FormArray, ValidatorFn,
...
import { PayChargeHistoryComponent } from "../pay-charge-history/pay-charge-history.component";
...
export class CreateSiteComponent implements OnInit {
  ...
  constructor(public modalService: NgbActiveModal, private siteService: SiteService, private modal: NgbModal,
    ...
    console.log("parent id", this.fromParent)
  }
  ...
  generateTimePickerFields1(noOfCalls: number): void {
    const exparticularTimesArray = this.AddCustomrSite.get("extern_particular_times") as FormArray;
  ...
  generateTimePickerFields2(noOfCalls: number): void {
    const intermedparticularTimesArray = this.AddCustomrSite.get("intermed_particular_times") as FormArray;
  ...
  submitCustomerSite() {
    ...
    // if (!this.AddCustomrSite.value.site_start_date ||
    ...
    if (!this.AddCustomrSite.value.site_end_date ||
    ...
    if (this.AddCustomrSite.invalid) {
      ...
      this.siteService.addSite(this.AddCustomrSite.value).subscribe(
        (res) => {
          ...
          if (res.success) {
            this.trackAdmin.storeActivity("Location", this.customer_id ? "Update Location"
              : `Create Location with name ${this.AddCustomrSite.value.site_name}`,this.routeId)
            .subscribe(() => {
  ...
  editSite(id) {
    ...
    this.siteService.getSpecificSite(id).subscribe(({ success, data, site_traker }) => {
      if (success) {
        ...
        this.toast.toastNotification1("Something went wrong or data not found. Please try again later",
        "Request Incomplete!");
        this.spinner.hide();
      }
      if (data.site_tasks) {
  ...
  payRateHistory() {
    ...
    dialogPopup.setButtons([ new ButtonMaker("Close", "close", ButtonLayoutDisplay.DARK),]);
  ...
  isAnyFieldError(fieldName: string): boolean {
    return this.isFieldError(this.submitted, this.AddCustomrSite.controls['customer_id'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['site_type'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['site_name'], fieldName) ||
      // this.isFieldError(this.submitted, this.AddCustomrSite.controls['site_start_date'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['signin_radius'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['radius_alert'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['site_state'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['site_level'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['payrol'], fieldName) ||
      this.isFieldError(this.submitted, this.AddCustomrSite.controls['site_break'], fieldName);
  }
