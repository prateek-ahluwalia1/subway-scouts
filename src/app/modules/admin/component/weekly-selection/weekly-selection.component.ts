...
export class WeeklySelectionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.startDate.emit(this.selectedStartWeek);
    this.endDate.emit(this.selectedEndWeek);
    this.globals.selectedDate = `${this.selectedStartWeek.format('MMM D')} - ${this.selectedEndWeek.format('D, YYYY')}`;
  ...
  public selectDate(date: CalendarDate[]) {
    ...
    this.startDate.emit(weekStart);

    this.globals.getWeekDays = [];
    if (this.globals.weekType == 'site-two-weeks' || this.globals.weekType == 'guard-two-weeks') {
      this.selectedEndWeek = moment(date[0].mDate)
      const endOfWeek = this.selectedEndWeek.clone().add(2, 'weeks').subtract(1, 'day');
      this.endDate.emit(endOfWeek);
      for (var i = 0; i <= 13; i++) {
      ...
      this.globals.selectedDate = `${this.selectedStartWeek.format('MMM D')} - ${endOfWeek.format('D, YYYY')}`;
    } else {
