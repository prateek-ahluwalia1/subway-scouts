...
@Component({
  selector: 'app-fortnight-selection',
  templateUrl: './fortnight-selection.component.html',
  styleUrls: ['./fortnight-selection.component.scss']
})

export class FortnightSelectionComponent implements OnInit {
  ...
  private fillDates(currentMoment: moment.Moment) {
    ...
    const lastDayOfGrid = moment(currentMoment).endOf('month').subtract(lastOfMonth, 'days').add(14, 'days');
  ...
  public selectDate(date: CalendarDate[]) {
    ...
    this.selectedEndWeek = moment(date[13].mDate);
    var weekStart = this.selectedStartWeek.clone().startOf('isoWeek');
    this.globals.getTwo_WeeksDays = [];
    for (var i = 0; i <= 13; i++) {
       this.globals.getTwo_WeeksDays.push(moment(weekStart).add(i, 'days').format("ddd , DD/MM"));
     
     // this.globals.getWeekDays.push(moment(weekStart).add(i, 'days').format("ddd"));
    };
    console.log('this.globals.getTwo_WeeksDays',this.globals.getTwo_WeeksDays);
    
    this.selectedDate = `${ this.selectedStartWeek.format('MMM D')} - ${this.selectedEndWeek.format('D, YYYY')}`;
