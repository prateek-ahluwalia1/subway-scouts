...
public function fetchCustomerSites(Request $request)
{
 ...
 if($request->has('customer_id') && !empty($request->customer_id)){
    ...
    if($request->type == 'location'){
        ...
        $getPublicHolidays = DB::table('public_holidays')
            ->whereBetween('date', [
        ...
        $holidays = [];
        if($getPublicHolidays->isNotEmpty()) {
            foreach($getPublicHolidays as $holiday) {
                $holidays[] = [
...
//                             $addedHours = ($addedHours - $newCurrentHours) + $satHours;
