...
use App\Exports\ThirtySixHoursExport;
...
class ThirtySixHrsShiftReport extends Controller
{


    function generateThirtySixHoursShiftReport()
    {
        // return Excel::download(new InvoiceReportExport, 'invoice_report.xlsx');
        $filename = time() . '_36_hours_shift_report.xlsx';
        Excel::store(new ThirtySixHoursExport, 'excel/thirtysixhours/' . $filename, 'excels');
        return response()->json(['success' => true, 'message' => '36 Hours Shift Report.', 'path' =>
        'https://' . request()->getHttpHost() . '/excel/thirtysixhours/' . $filename]);
    }
    ...
    function getReportData($request)
    {
       if (!empty($request['date'])) {
        ...
        // restrict to Monday → Saturday
        $from = Carbon::parse($from)->startOfWeek(Carbon::MONDAY)->format('Y-m-d 00:00');
        $to = Carbon::parse($to)->endOfWeek(Carbon::SATURDAY)->format('Y-m-d 23:59');
    ...
    private function getShiftHours ($start, $end, $siteID = null, $public_holiday = null, $ph_duration = null) {
        ...
        {
            $sat_end = strtotime(date('m/d/Y 23:59', $start));
            $diff = $sat_end - $start;
        ...
        {
            $sat_start = strtotime(date('m/d/Y 00:00', $end));
            $diff = $end - $sat_start;
            $total_saturday_hours = round($diff / ( 60 * 60 ), 2);
        ...
        {
            $sun_end = strtotime(date('m/d/Y 23:59', $start));
            $diff = $sun_end - $start;
        ...
        }elseif($day_start != 'Sunday' && $day_end == 'Sunday')
        {
            ...
            $sun_start = strtotime(date('m/d/Y 00:00', $end));
            // $diff = $end - $sun_start;
            // $total_sunday_hours = round($diff / ( 60 * 60 ), 2);
