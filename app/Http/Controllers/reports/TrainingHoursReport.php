...
use App\Exports\TrainingHoursExport;
...
class TrainingHoursReport extends Controller
{


    function generateTrainingHoursReport()
    {
        // return Excel::download(new InvoiceReportExport, 'invoice_report.xlsx');
        $filename = time() . '_training_hours_report.xlsx';
        Excel::store(new TrainingHoursExport, 'excel/traininghours/' . $filename, 'excels');
        return response()->json(['success' => true, 'message' => 'Award Overtime.', 'path' =>
        'https://' . request()->getHttpHost() . '/excel/traininghours/' . $filename]);
    }
        ...
        if (isset($request['date']) && $request['date'] != '') {
            ...
            for ($i = 0; $i <= $weeksDifference; $i++) {
                ...
                $weeks = JobRoster::whereNotNull('guard_id')
                    ->where(function ($q) {
                        $q->where('job_status', 'completed')->where('training', 1)
