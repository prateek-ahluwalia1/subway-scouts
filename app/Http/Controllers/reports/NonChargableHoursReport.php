...
use App\Exports\NonChargableHoursExport;
...
class NonChargableHoursReport extends Controller
{


    function generateNonChargeableHoursReport()
    {
        // return Excel::download(new InvoiceReportExport, 'invoice_report.xlsx');
        $filename = time() . '_non_chargable_hours_report.xlsx';
        Excel::store(new NonChargableHoursExport, 'excel/nonchargablehours/' . $filename, 'excels');
        return response()->json(['success' => true, 'message' => 'Award Overtime.', 'path' =>
        'https://' . request()->getHttpHost() . '/excel/nonchargablehours/' . $filename]);
    }
        ...
        if (isset($request['date']) && $request['date'] != '') {
            ...
            for ($i = 0; $i <= $weeksDifference; $i++) {
                ...
                $weeks = JobRoster::whereNotNull('guard_id')
                    ->where(function ($q) {
                        $q->where('job_status', 'completed')->where('shift_chargeable', 'no')
