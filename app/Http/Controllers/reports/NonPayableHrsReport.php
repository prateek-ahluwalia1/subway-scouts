...
use App\Exports\NonPayableHrsExport;
...
class NonPayableHrsReport extends Controller
{


    function generateNonPayableHrsReport()
    {
        // return Excel::download(new InvoiceReportExport, 'invoice_report.xlsx');
        $filename = time() . '_non_payable_hours_report.xlsx';
        Excel::store(new NonPayableHrsExport, 'excel/nonpayablehours/' . $filename, 'excels');
        return response()->json(['success' => true, 'message' => 'Award Overtime.', 'path' =>
        'https://' . request()->getHttpHost() . '/excel/nonpayablehours/' . $filename]);
    }
        ...
        if (isset($request['date']) && $request['date'] != '') {
            ...
            for ($i = 0; $i <= $weeksDifference; $i++) {
                ...
                $weeks = JobRoster::whereNotNull('guard_id')
                    ->where(function ($q) {
                        $q->where('job_status', 'completed')->where('shift_payable', 'no')
