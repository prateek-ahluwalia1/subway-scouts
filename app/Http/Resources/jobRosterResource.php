...
class jobRosterResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        if ($this->update_status == 1) {
            DB::table('job_rosters')->where('id', $this->id)->update(['update_status' => 0]);
        }
        $completedTasks = $this->jobRosterTask->filter(function($task) {
        ...
        return [
            ...
            'status' => $is_status, //
            'shiftTasksCount' => $this->jobRosterTask->count(),
